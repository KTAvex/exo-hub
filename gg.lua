...
local function buildHotkeyList()
    ...
    for i, entry in ipairs(entries) do
        ...
        local accentCol = entry.isBoost and (cfg.boostOn and C.on or C.off) or C.cyan
        ins(makeRect(HX+4, ry+1, 3, ROW-2, accentCol, true))

        local labelCol = entry.isBoost and (cfg.boostOn and C.on or C.off) or C.text
...
task.spawn(function()
    while true do
        ...
        do
            ...
            local entryCount = 4 + #cfg.customMacros
        ...
        local now = os.clock()
        if (state.dragging or state.resizing or state.editorDragging or state.editorResizing) and (now - lastRebuildTime > 0.05) then
            rebuildMenu()
            tabBuilders[state.currentTab]()
            buildMacroEditor()
            lastRebuildTime = now
